# index.py — Ask Larry proxy using LangChain + LangSmith auto tracing
...
# Optional domain-limited search (enable by setting TAVILY_API_KEY)
...
and pages linked from that site. Prefer primary sources and include at least one
source link in your answer.

Answer concisely in Markdown. If you cannot find an answer on allowed pages,
say you’re not sure and suggest where to look on {DOMAIN}.
"""

# ---------- In-memory rate limiting (resets on cold start) ----------
RATE_LIMIT = int(os.environ.get("ASK_LARRY_RATE_LIMIT", "10"))   # requests
RATE_WINDOW = int(os.environ.get("ASK_LARRY_RATE_WINDOW", "60")) # seconds
...
def build_chain():
    ...
    if USE_TAVILY:
        ...
        tools = [search]
        agent = create_openai_tools_agent(llm, tools, base_prompt)
        return AgentExecutor(agent=agent, tools=tools, verbose=False)

    # Simple (no tools) chain
...
def polish_markdown(md: str) -> str:
    # remove trailing "(letourdeshore.com)"
...
def ask():
    ...
    if any(b in ua.lower() for b in ("mozilla", "safari", "chrome")):
        return jsonify(error="forbidden", detail="Browser access blocked"), 403

    if rate_limited(instance_id):
    ...
    try:
        if USE_TAVILY:
            ...
            # AgentExecutor returns a dict { "output": "...", ... }
            text = result.get("output") if isinstance(result, dict) else str(result)
        else:
            # Simple chain returns a string
            text = CHAIN.invoke({"question": question})

        text = polish_markdown(text or "")
        return jsonify({"text": text})

    except Exception as e:
        # Surface a clean error to the app
