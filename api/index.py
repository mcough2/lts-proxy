...
def _app_token_ok() -> bool:
    """NEW: if APP_TOKEN is set and client sends matching X-App-Token, accept immediately."""
    if not APP_TOKEN:
        return False
...
@app.post("/ask")
def ask():
    # NEW: If the shared app token is valid, skip iOS header checks.
    if not _app_token_ok():
        # Gate / rate limit (original behavior)
